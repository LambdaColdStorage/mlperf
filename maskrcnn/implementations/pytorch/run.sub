...
#SBATCH --ntasks-per-node=2	# n tasks per machine (one task per gpu)
#SBATCH --threads-per-core=1	# HT is on
#SBATCH --cores-per-socket=10	# 20 cores on each socket 


DATESTAMP=${DATESTAMP:-`date +'%y%m%d%H%M%S%N'`}

## Data, container and volumes
BENCHMARK=${BENCHMARK:-"maskrcnn"}
BENCHMARK_NAME="MASKRCNN"
CONT=${CONT:-"mlperf-lambda:$BENCHMARK"}
DATADIR=${DATADIR:-"/home/data/mlpef/object_detection/coco-2017"}
